...
def ec_file_sizes_to_csv(mrt_files: tuple[MRTFile, ...], output_path: Path) -> None:
    """
...
def ec_file_sizes_from_csv(mrt_files: tuple["MRTFile", ...], input_path: Path) -> None:
    """
    ...
    with input_path.open("r", newline="") as f:
        ...
        url_to_size = {row["url"]: int(row["ec_file_size"]) for row in reader}
